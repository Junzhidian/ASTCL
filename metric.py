...
def mape_error(pred, label, missing_value=0):
    if pred.device != label.device:
        label = label.to(pred.device)
    return (((pred - label) / (label + 0.1)) * get_valid(label, missing_value)).abs().sum()
...
class MetricMAPE(Metric):
    ...
    def update(self, pred, label):
        self.cnt += num_valid(label)
        self.value += mape_error(pred, label)

    def get_value(self):
        return (self.value / (self.cnt + 1e-8)).item() * 100
